...
<div id="laporan-print" class="max-w-5xl mx-auto px-4 py-10">
    ...
    <div class="no-print mb-8 bg-white p-5 rounded-2xl border border-gold/20 shadow-sm">
        ...
        <form id="filter-form" method="GET" class="flex flex-wrap items-end gap-3">
            <div>
                ...
                <input type="month" name="dari_bulan" value="{{ $dariBulan }}" class="border rounded-lg px-3 py-2 text-sm">
            </div>
            <div>
                <label class="block text-xs font-bold text-indigoCustom mb-1">Sampai Bulan</label>
                <input type="month" name="sampai_bulan" value="{{ $sampaiBulan }}" class="border rounded-lg px-3 py-2 text-sm">
            </div>
            ...
            @if($dariBulan || $sampaiBulan)
                <a href="{{ route('admin.laporan') }}" class="text-xs text-soga underline">Reset</a>
            @endif
        </form>
    </div>
    ...
    <div class="mb-8 text-center border-b border-gold/30 pb-4">
        ...
        <p class="text-xs text-ink/50 mt-1">
            Periode:
            {{ $dariBulan ? \Carbon\Carbon::parse($dariBulan.'-01')->translatedFormat('F Y') : 'Awal' }}
            s/d
            {{ $sampaiBulan ? \Carbon\Carbon::parse($sampaiBulan.'-01')->translatedFormat('F Y') : 'Sekarang' }}
            &middot; Dicetak: {{ now()->translatedFormat('d F Y, H:i') }} WIB
        </p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-indigo-50 p-6 rounded-2xl border border-indigo-100">
            ...
            <h2 class="text-2xl font-bold text-soga mt-2">Rp {{ number_format($totalPendapatan, 0, ',', '.') }}</h2>
        </div>
        <div class="bg-indigo-50 p-6 rounded-2xl border border-indigo-100">
            <p class="text-indigoCustom font-bold text-sm">Total Transaksi</p>
            <h2 class="text-2xl font-bold text-soga mt-2">{{ $totalTransaksi }}</h2>
        </div>
        <div class="bg-indigo-50 p-6 rounded-2xl border border-indigo-100">
            <p class="text-indigoCustom font-bold text-sm">Produk Terjual</p>
            <h2 class="text-2xl font-bold text-soga mt-2">{{ $totalProdukTerjual }} pcs</h2>
        </div>
    </div>
        ...
        <table class="w-full text-left text-xs">
            ...
            <tbody>
                ...
                <tr class="border-b last:border-0 align-top">
                    <td class="py-3 font-semibold">{{ $item->kode_transaksi }}</td>
                    <td class="py-3">{{ $item->pesanan->user->nama ?? 'User Tidak Ditemukan' }}</td>
                    <td class="py-3">
                        @foreach($item->details as $detail)
                            {{ $detail->produk->nama_produk ?? 'Produk dihapus' }} (x{{ $detail->jumlah }})<br>
                        @endforeach
                    </td>
                    <td class="py-3 text-right font-bold text-soga">Rp {{ number_format($item->total_bayar, 0, ',', '.') }}</td>
                </tr>
                @empty
